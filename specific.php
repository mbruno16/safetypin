...
$fullname = DB::query('SELECT users.`fullname`,posts.id FROM users,posts WHERE users.id = posts.user_id AND posts.id = :id', array(':id'=>$id))[0]['fullname'];
$verified = DB::query('SELECT users.verified FROM users,posts WHERE users.id = posts.user_id AND posts.id = :id', array(':id'=>$id))[0]['verified'];
$phonenumber = DB::query('SELECT users.cnumber FROM users,posts WHERE users.id = posts.user_id AND posts.id = :id', array(':id'=>$id))[0]['cnumber'];
$email = DB::query('SELECT users.email FROM users,posts WHERE users.id = posts.user_id AND posts.id = :id', array(':id'=>$id))[0]['email'];
$postusername = DB::query('SELECT users.username FROM users,posts WHERE users.id = posts.user_id AND posts.id = :id', array(':id'=>$id))[0]['username'];

if (isset($_GET['username'])) {
        if (DB::query('SELECT username FROM users WHERE username=:username', array(':username'=>$_GET['username']))) {

                $username = DB::query('SELECT username FROM users WHERE username=:username', array(':username'=>$_GET['username']))[0]['username'];
                $phonenumber = DB::query('SELECT cnumber FROM users WHERE username=:username', array(':username'=>$_GET['username']))[0]['cnumber'];
                $fullname = DB::query('SELECT fullname FROM users WHERE username=:username', array(':username'=>$_GET['username']))[0]['fullname'];
...
<head>
    ...
    <title>Safety Pin | <?php echo $fullname; ?>'s Post</title>
    <meta
      name="description"           
      content="This is the profile page where users can see only the posts of a specific user.">
...
<body onload="animatedCursor();">
    ...
    <div>
        <div class="container">
            <h1><?php echo $fullname; ?>'s Post <?php if ($verified) { echo '<i class="glyphicon glyphicon-ok-sign verified" data-toggle="tooltip" title="Verified User" style="font-size:28px;color:#e61c5d;"></i>'; } ?></h1>

            <div class="row">
                <div class="col-md-3">
                    <ul class="list-group">
                        <li class="list-group-item" style="margin-bottom:30px; border-radius:10px; padding-bottom:0px;"><span><strong>About this Post</strong></span>
                            <p><br>Post Owner's Username: @<?php echo $postusername; ?></p>
                            <p>Post Owner's Contact Number: <?php echo $phonenumber; ?> <a href="tel:'<?php echo $phonenumber; ?>'"><i class="glyphicon glyphicon-earphone" style="display:inline; float: right; font-size:15px;"></i></a></p>
                            <p>Post Owner's Email Address: <?php echo $email; ?> <a href="mailto:<?php echo $email; ?>"><i class="glyphicon glyphicon-envelope" style="display:inline; float: right; font-size:15px;"></i></a></p>
                        </li>
                    </ul>
                </div>
                <div class="col-md-6">
            ...
            if ($(this).scrollTop() + 1 >= $('body').height() - $(window).height()) {
                    if (working == false) {
                            ...
                            $.ajax({
                                    ...
                                    type: "GET",
                                    url: "api/specific?username=<?php echo $username; ?>&start="+start+"&id="+id,
                                    processData: false,
                                    ...
                                    success: function(r) {
                                            ...
                                            $.each(posts, function(index) {
                                                    ...
                                                    if (posts[index].PostImage == "") { //added condition so that if post have img will post correctly
                                                        $('.timelineposts').html(
                                                            $('.timelineposts').html() + 
                                                            //added style to list-group-item
                                                            '<ul class="list-group"><li class="list-group-item" style="margin-top:5px; border-radius:10px;" id="'+posts[index].PostId+'"><p style="font-size:18px; margin-bottom:10px;"><i class="glyphicon glyphicon-map-marker" data-toggle="tooltip" title="Posted From" style="font-size:20px;color:#e61c5d;"></i>&nbsp<a href="'+posts[index].Location_Link+'" target="_blank" rel="noopener noreferrer">'+posts[index].Location+'</a></p><blockquote><p>'+posts[index].PostBody+'</p><footer>By <a href="profile.php?username='+posts[index].Username+'"> '+posts[index].PostedBy+' </a> <?php if ($verified) { echo '<i class="glyphicon glyphicon-ok-sign verified" data-toggle="tooltip" title="Verified User" style="font-size:16px;color:#e61c5d;"></i>'; } ?> on '+posts[index].PostDate+'</footer></blockquote><button class="btn btn-default" type="button" style="background-image:url(&quot;none&quot;);background-color:transparent;" data-id=\"'+posts[index].PostId+'\"><i class="glyphicon glyphicon-circle-arrow-up" style="color:'+posts[index].LColor+'"></i><span style="color:'+posts[index].LColor+'"> '+posts[index].Likes+' Upvote</span></button><button class="btn btn-default" type="button" style="background-image:url(&quot;none&quot;);background-color:transparent;" data-id1=\"'+posts[index].PostId+'\"><i class="glyphicon glyphicon-circle-arrow-down" style="color:'+posts[index].DColor+'"></i><span style="color:'+posts[index].DColor+'"> '+posts[index].Dislikes+' Downvote</span></button><button name="comment" class="btn btn-default comment" data-postid=\"'+posts[index].PostId+'\" type="button" style="color:background-image:url(&quot;none&quot;);background-color:transparent;"><i class="glyphicon glyphicon-comment" style="color:#fdc405;"></i><span style="color:#fdc405;">&nbsp'+posts[index].Commented+' Comment/s</span></button></li></ul>'
                                                            )
                                                    } else {
                                                            $('.timelineposts').html( //added css to img scr max-height
                                                                $('.timelineposts').html() +

                                                            '<ul class="list-group"><li class="list-group-item" style="margin-top:5px; border-radius:10px;" id=""><p style="font-size:18px; margin-bottom:10px;"><i class="glyphicon glyphicon-map-marker" data-toggle="tooltip" title="Posted From" style="font-size:20px;color:#e61c5d;"></i>&nbsp<a href="'+posts[index].Location_Link+'" target="_blank" rel="noopener noreferrer">'+posts[index].Location+'</a></p><blockquote><p>'+posts[index].PostBody+'</p><img src="" data-tempsrc="'+posts[index].PostImage+'" class="postimg" id="img'+posts[index].postId+'" alt="post image"><footer>By <a href="profile.php?username='+posts[index].Username+'"> '+posts[index].PostedBy+' </a> <?php if ($verified) { echo '<i class="glyphicon glyphicon-ok-sign verified" data-toggle="tooltip" title="Verified User" style="font-size:16px;color:#e61c5d;"></i>'; } ?> on '+posts[index].PostDate+'</footer></blockquote><button class="btn btn-default" type="button" style="background-image:url(&quot;none&quot;);background-color:transparent;" data-id=\"'+posts[index].PostId+'\"><i class="glyphicon glyphicon-circle-arrow-up" style="color:'+posts[index].LColor+'"></i><span style="color:'+posts[index].LColor+'"> '+posts[index].Likes+' Upvote</span></button><button class="btn btn-default" type="button" style="background-image:url(&quot;none&quot;);background-color:transparent;" data-id1=\"'+posts[index].PostId+'\"><i class="glyphicon glyphicon-circle-arrow-down" style="color:'+posts[index].DColor+'"></i><span style="color:'+posts[index].DColor+'"> '+posts[index].Dislikes+' Downvote</span></button><button name="comment" class="btn btn-default comment" data-postid=\"'+posts[index].PostId+'\" type="button" style="background-image:url(&quot;none&quot;);background-color:transparent;"><i class="glyphicon glyphicon-comment" style="color:#fdc405;"></i><span style="color:#fdc405;">&nbsp'+posts[index].Commented+' Comment/s</span></button></li></ul>'
                                                            )
                                                    }
    ...
    var aTag = $(aid);
        ...
        $(document).ready(function() {
                ...
                $.ajax({
                        ...
                        type: "GET",
                        url: "api/specific?username=<?php echo $username; ?>&start=0&id=<?php echo $id; ?>",
                        processData: false,
                        contentType: "application/json",
                        data: '',
                        success: function(r) {
